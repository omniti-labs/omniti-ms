...
UPDATE=60
BUILD=28
VERHUMAN="jdk8u${UPDATE}-b${BUILD}"

PKG=omniti/runtime/java
SUMMARY="Open-source implementation of the eigth edition of the Java SE Platform"
DESC="$SUMMARY"
DESTDIR=$J2RE_INSTALLTMP
CONFIGURE_OPTS="--with-target-bits=64 \
  --x-includes=$TMPDIR/openwin/X11/include \
  --x-libraries=$TMPDIR/openwin/X11/lib \
  --with-cups-include=$TMPDIR/include/cups \
  --with-freetype-include=/opt/omni/include/freetype2 \
  --with-freetype-lib=/opt/omni/lib/amd64"
...
HGREV=581a6452226f60b7e5d78c1bb0a5f294bd7c0b83
...
REPO="http://hg.openjdk.java.net/jdk8u/jdk8u60/"
...
configure64() {
    ...
    PATH="/opt/sunstudio12.1/bin:/usr/bin:/usr/sbin:/sbin:/usr/gnu/bin:/opt/omni/bin:/opt/sunstudio12.1/bin"
...
build64() {
    pushd $TMPDIR/$BUILDDIR > /dev/null
    logcmd chmod 755 configure 
    unset PATH
    export PATH="/usr/bin:/usr/sbin:/sbin:/usr/gnu/bin:/opt/omni/bin:/opt/sunstudio12.1/bin"
    logcmd export
    configure64
...
make_install_j2re() {
    ...
    JAVA_INSTALL_ROOT=$J2RE_INSTALLTMP/opt/java

    logmsg "Installing JRE to $J2RE_INSTALLTMP"

    # make our base directories under /opt
    logcmd mkdir -p $JAVA_INSTALL_ROOT
    logcmd mkdir -p $J2RE_INSTALLTMP/opt/java/bin
...
make_install_j2sdk() {
    ...
    JAVA_INSTALL_ROOT=$J2SDK_INSTALLTMP/opt/java

    logmsg "Installing SDK to $J2SDK_INSTALLTMP"

    # make our base directories under /usr
    logcmd mkdir -p $JAVA_INSTALL_ROOT
    logcmd mkdir -p $J2SDK_INSTALLTMP/opt/java/bin
