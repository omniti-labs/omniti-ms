...
VER=7.1.3
VERHUMAN=$VER
PKG=omniti/runtime/php-71
SUMMARY="PHP Server 7.1"
...
install_conffiles() {
    ...
    logcmd cp $SRCDIR/files/php.ini $DESTDIR/opt/php71/lib/php.ini

    logcmd cp $SRCDIR/files/php-fpm.conf $DESTDIR/opt/php71/etc/php-fpm.conf
    logcmd mkdir -p $DESTDIR/opt/php71/etc/php-fpm.d
    logcmd cp $SRCDIR/files/www.conf $DESTDIR/opt/php71/etc/php-fpm.d/www.conf

    logcmd mkdir -p $DESTDIR/var/svc/manifest/application
    logcmd cp $SRCDIR/files/php71-fpm.xml \
        $DESTDIR/var/svc/manifest/application/php71-fpm.xml
}
